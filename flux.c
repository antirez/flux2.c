 ...
 * Ties together all components: tokenizer, text encoder, VAE, transformer,
 * sampling.
 */
...
extern float *flux_vae_encode(flux_vae_t *vae, const float *img, int batch,
...
extern flux_transformer_t *
flux_transformer_load_safetensors(safetensors_file_t *sf);
extern void flux_transformer_free(flux_transformer_t *tf);
extern float *flux_transformer_forward(flux_transformer_t *tf,
                                       const float *img_latent, int img_h,
                                       int img_w, const float *txt_emb,
                                       int txt_seq, float timestep);

extern float *flux_sample_euler(void *transformer, void *text_encoder, float *z,
                                int batch, int channels, int h, int w,
                                const float *text_emb, int text_seq,
                                const float *null_emb, const float *schedule,
...
extern float *flux_init_noise(int batch, int channels, int h, int w,
                              int64_t seed);
...
const char *flux_get_error(void) { return g_error_msg; }
...
flux_ctx *flux_load_dir(const char *model_dir) {
  ...
  /* Load VAE */
  snprintf(path, sizeof(path), "%s/vae/diffusion_pytorch_model.safetensors",
           model_dir);
  if (file_exists(path)) {
...
float *flux_encode_text(flux_ctx *ctx, const char *prompt, int *out_seq_len) {
  ...
  if (!ctx->qwen3_encoder && ctx->model_dir[0]) {
    if (flux_phase_callback)
      flux_phase_callback("reloading encoder", 0);
    ctx->qwen3_encoder = qwen3_encoder_load(ctx->model_dir);
    if (flux_phase_callback)
      flux_phase_callback("reloading encoder", 1);
  ...
  if (flux_phase_callback)
    flux_phase_callback("encoding text", 0);
  float *embeddings = qwen3_encode_text(ctx->qwen3_encoder, prompt);
  if (flux_phase_callback)
    flux_phase_callback("encoding text", 1);
...
flux_image *flux_generate(flux_ctx *ctx, const char *prompt,
  ...
  if (p.height <= 0)
    p.height = 1024;
  if (p.num_steps <= 0)
    p.num_steps = 4; /* Klein default */
  if (p.guidance_scale <= 0)
    p.guidance_scale = 1.0f;
  ...
  /* Sample */
  float *latent =
      flux_sample_euler(ctx->transformer, ctx->qwen3_encoder, z, 1,
                        FLUX_LATENT_CHANNELS, latent_h, latent_w, text_emb,
                        text_seq, NULL, /* No null embedding for klein */
                        schedule, p.num_steps, p.guidance_scale, NULL);
...
flux_image *flux_generate_with_embeddings(flux_ctx *ctx, const float *text_emb,
  ...
  /* Sample */
  float *latent = flux_sample_euler(ctx->transformer, ctx->qwen3_encoder, z, 1,
                                    FLUX_LATENT_CHANNELS, latent_h, latent_w,
                                    text_emb, text_seq, NULL, schedule,
                                    p.num_steps, p.guidance_scale, NULL);
...
flux_image *flux_img2img(flux_ctx *ctx, const char *prompt,
  ...
  if (ctx->vae) {
    img_latent = flux_vae_encode(ctx->vae, img_tensor, 1, p.height, p.width,
                                 &latent_h, &latent_w);
  } else {
    /* Placeholder if no VAE */
    latent_h = p.height / 16;
    latent_w = p.width / 16;
    img_latent = (float *)calloc(FLUX_LATENT_CHANNELS * latent_h * latent_w,
                                 sizeof(float));
  }
  ...
  float *latent = flux_sample_euler(
      ctx->transformer, ctx->qwen3_encoder, img_latent, 1, FLUX_LATENT_CHANNELS,
      latent_h, latent_w, text_emb, text_seq, NULL, schedule, num_steps,
  ...
  if (ctx->vae) {
    if (flux_phase_callback)
      flux_phase_callback("decoding image", 0);
    result = flux_vae_decode_gpu(ctx->vae, latent, 1, latent_h, latent_w);
    if (flux_phase_callback)
      flux_phase_callback("decoding image", 1);
  }
...
void flux_set_seed(int64_t seed) { flux_rng_seed((uint64_t)seed); }
...
const char *flux_model_info(flux_ctx *ctx) {
  ...
  snprintf(info, sizeof(info), "%s v%s (max %dx%d, %d steps)", ctx->model_name,
           ctx->model_version, ctx->max_width, ctx->max_height,
           ctx->default_steps);
  return info;
}
...
float *flux_encode_image(flux_ctx *ctx, const flux_image *img, int *out_h,
  ...
  float *latent = flux_vae_encode(ctx->vae, tensor, 1, img->height, img->width,
                                  out_h, out_w);
  free(tensor);
  return latent;
}
...
flux_image *flux_decode_latent(flux_ctx *ctx, const float *latent, int latent_h,
  ...
  if (flux_phase_callback)
    flux_phase_callback("decoding image", 0);
  flux_image *img =
      flux_vae_decode_gpu(ctx->vae, latent, 1, latent_h, latent_w);
  if (flux_phase_callback)
    flux_phase_callback("decoding image", 1);
  return img;
}
...
float *flux_denoise_step(flux_ctx *ctx, const float *z, float t,
                         ...
                         int latent_w) {
  if (!ctx || !z || !text_emb || !ctx->transformer)
    return NULL;

  return flux_transformer_forward(ctx->transformer, z, latent_h, latent_w,
                                  text_emb, text_len, t);
}
